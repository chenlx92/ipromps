...
import iprompslib_emg_joint_3_task
import scipy.linalg
# import rospy

plt.close('all')    # close all windows
len_normal = 101    # the len of normalized traj
nrDemo = 5         # number of trajectoreis for training
...
ipromp_aluminum_hold = iprompslib_emg_joint_3_task.IProMP(num_joints=15, nrBasis=11, sigma=0.05, num_samples=101)
ipromp_spanner_handover = iprompslib_emg_joint_3_task.IProMP(num_joints=15, nrBasis=11, sigma=0.05, num_samples=101)
ipromp_tape_hold = iprompslib_emg_joint_3_task.IProMP(num_joints=15, nrBasis=11, sigma=0.05, num_samples=101)

# add demostration
for idx in range(0, nrDemo):
    # add demonstration of aluminum_hold
    demo_temp = np.hstack([train_set_aluminum_hold_emg_norm_full[idx], train_set_aluminum_hold_pose_norm_full[idx]])
    ipromp_aluminum_hold.add_demonstration(demo_temp)
    # add demonstration of spanner_handover
    demo_temp = np.hstack([train_set_spanner_handover_emg_norm_full[idx], train_set_spanner_handover_pose_norm_full[idx]])
    ipromp_spanner_handover.add_demonstration(demo_temp)
    # add demonstration of tape_hold
    demo_temp = np.hstack([train_set_tape_hold_emg_norm_full[idx], train_set_tape_hold_pose_norm_full[idx]])
    ipromp_tape_hold.add_demonstration(demo_temp)



# # spanner handover
test_set = np.hstack((train_set_spanner_handover_emg_norm21, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm22, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm23, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm24, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm25, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm26, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm27, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm28, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_spanner_handover_emg_norm29, np.zeros([len_normal, 7])))
# aluminum hold
test_set = np.hstack((train_set_aluminum_hold_emg_norm21, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm22, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm23, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm24, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm25, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm26, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm27, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm28, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_aluminum_hold_emg_norm29, np.zeros([len_normal, 7])))
# tape hold
test_set = np.hstack((train_set_tape_hold_emg_norm21, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm22, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm23, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm24, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm25, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm26, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm27, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm28, np.zeros([len_normal, 7])))
test_set = np.hstack((train_set_tape_hold_emg_norm29, np.zeros([len_normal, 7])))

# add via point as observation
emg_meansurement_noise_cov = np.eye((8))*1000
pose_meansurement_noise_cov = np.eye((7))*1.0
meansurement_noise_cov_full = scipy.linalg.block_diag(emg_meansurement_noise_cov, pose_meansurement_noise_cov)
