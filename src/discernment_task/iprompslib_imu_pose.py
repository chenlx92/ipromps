...
class NDProMP(object):
    ...
    def add_demonstration(self, demonstration):
        ...
        :return:
        """
        demonstration = np.array(
            demonstration).T  # Revert the representation for each time for each joint, for each joint for each time

        if len(demonstration) != self.num_joints:
            raise ValueError(
                "The given demonstration has {} joints while num_joints={}".format(len(demonstration), self.num_joints))

        for joint_demo_idx, joint_demo in enumerate(demonstration):
            self.promps[joint_demo_idx].add_demonstration(joint_demo)

        self.demo_W_full = np.hstack([self.promps[0].W, self.promps[1].W, self.promps[2].W, self.promps[3].W,
                                      self.promps[4].W, self.promps[5].W, self.promps[6].W, self.promps[7].W,
                                      self.promps[8].W, self.promps[9].W, self.promps[10].W])
        ...
        self.cov_W_full = self.cov_W_full.reshape([self.num_joints * self.nrBasis, self.num_joints * self.nrBasis]) if \
        self.promps[0].nrTraj > 1 else None
    ...
    def set_goal(self, obsy, sigmay=1e-6):
        if len(obsy) != self.num_joints:
            raise ValueError(
                "The given goal state has {} joints while num_joints={}".format(len(obsy), self.num_joints))
    ...
    def set_start(self, obsy, sigmay=1e-6):
        if len(obsy) != self.num_joints:
            raise ValueError(
                "The given start state has {} joints while num_joints={}".format(len(obsy), self.num_joints))
    ...
    def plot_updated(self, x=None, output_randomess=1e-105):
        mean0 = np.dot(self.promps[0].Phi.T, self.promps[0].meanW_updated)
        std0 = 2 * np.sqrt(
            np.diag(np.dot(self.promps[0].Phi.T, np.dot(self.promps[0].sigmaW_updated, self.promps[0].Phi))))
        plt.fill_between(x, mean0 - std0, mean0 + std0, color='r', alpha=0.4)

        mean1 = np.dot(self.promps[1].Phi.T, self.promps[1].meanW_updated)
        std1 = 2 * np.sqrt(
            np.diag(np.dot(self.promps[1].Phi.T, np.dot(self.promps[1].sigmaW_updated, self.promps[1].Phi))))
        plt.fill_between(x, mean1 - std1, mean1 + std1, color='b', alpha=0.4)
...
class ProMP(object):
    ...
    def __init__(self, nrBasis=11, sigma=0.05, num_samples=101):
        ...
        self.Phi = np.exp(-.5 * (
        np.array(map(lambda x: x - self.C, np.tile(self.x, (self.nrBasis, 1)).T)).T ** 2 / (self.sigma ** 2)))
    ...
    def add_demonstration(self, demonstration):
        ...
        self.W = np.dot(np.linalg.inv(np.dot(self.Phi, self.Phi.T)),
                        np.dot(self.Phi, self.Y.T)).T  # weights for each trajectory, MLE here
    ...
    def generate_trajectory(self, randomness=1e-10):
        ...
        for viapoint in self.viapoints:
            PhiT = np.exp(-.5 * (
            np.array(map(lambda x: x - self.C, np.tile(viapoint['t'], (11, 1)).T)).T ** 2 / (self.sigma ** 2)))
            PhiT = PhiT / sum(PhiT)  # basis functions at observed time points

            # Conditioning
            aux = viapoint['sigmay'] + np.dot(np.dot(PhiT.T, newSigma), PhiT)
            K = np.dot(newSigma, PhiT) * 1 / aux
            newMu = newMu + np.dot(K, (
            viapoint['obsy'] - np.dot(PhiT.T, newMu)))  # new weight mean conditioned on observations
    ...
    def plot_updated(self, x=None, legend='', color='b', via_show=True):
        ...
        if via_show == True:
            for viapoint_id, viapoint in enumerate(self.viapoints):
                x_index = x[int(round((len(x) - 1) * viapoint['t'], 0))]
                plt.plot(x_index, viapoint['obsy'], marker="o", markersize=10, label="Via {}".format(viapoint_id),
                         color=color)
        ...
        for num_viapoint in range(len(self.promps[0].viapoints)):
            ...
            y_observed = np.array(
                [[self.promps[0].viapoints[num_viapoint]['obsy']], [self.promps[1].viapoints[num_viapoint]['obsy']],
                 ...
                 [self.promps[6].viapoints[num_viapoint]['obsy']], [self.promps[7].viapoints[num_viapoint]['obsy']],
                 [self.promps[8].viapoints[num_viapoint]['obsy']], [self.promps[9].viapoints[num_viapoint]['obsy']],
                 [self.promps[10].viapoints[num_viapoint]['obsy']]])
    ...
    def prob_obs(self):
        ...
        for obs in self.obs:
            PhiT = np.exp(-.5 * (np.array(map(lambda x: x - self.promps[0].C, np.tile(np.int(obs['t']),
                                                                                      (self.nrBasis, 1)).T)).T ** 2 / (
                                 self.promps[0].sigma ** 2)))
            PhiT = PhiT / sum(PhiT)  # basis functions at observed time points
            # here is a trick for construct the observation matrix
            zero_entry = np.zeros([1, 11])
            PhiT_full = scipy.linalg.block_diag(PhiT.T, PhiT.T, PhiT.T, PhiT.T,
                                                zero_entry, zero_entry, zero_entry,
                                                zero_entry, zero_entry, zero_entry, zero_entry)
