...
for idx in range(0, nrDemo):
    ...
    demo_temp = np.hstack([dataset_aluminum_hold_norm[idx]["imu"], dataset_aluminum_hold_norm[idx]["pose"]])
    ipromp_aluminum_hold.add_demonstration(demo_temp)
    # add demonstration of spanner_handover
    demo_temp = np.hstack([dataset_spanner_handover_norm[idx]["imu"], dataset_spanner_handover_norm[idx]["pose"]])
    ipromp_spanner_handover.add_demonstration(demo_temp)
    # add demonstration of tape_hold
    demo_temp = np.hstack([dataset_tape_hold_norm[idx]["imu"], dataset_tape_hold_norm[idx]["pose"]])
...
# aluminum hold
test_set = np.hstack((dataset_aluminum_hold_norm[20]["imu"], np.zeros([len_normal, 7])))
robot_response = dataset_aluminum_hold_norm[20]["pose"]
# spanner handover
test_set = np.hstack((dataset_spanner_handover_norm[20]["imu"], np.zeros([len_normal, 7])))
robot_response = dataset_spanner_handover_norm[20]["pose"]
# tape hold
test_set = np.hstack((dataset_tape_hold_norm[20]["imu"], np.zeros([len_normal, 7])))
