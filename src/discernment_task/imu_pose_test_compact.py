...
right_id = 2; selected_testset = dataset_tape_hold_norm[np.int(sys.argv[1])]
...
if b_plot_prior_distribution == True:
    ...
    for i in range(4):
        plt.subplot(411+i)
        ipromp_aluminum_hold.promps[i].plot(color='b', legend='alumnium hold model, imu'); plt.axis('off')
    plt.figure(52)
    for i in range(7):
        plt.subplot(711+i)
        ipromp_aluminum_hold.promps[4+i].plot(color='b', legend='alumnium hold model, pose'); plt.axis('off')
    # plot ipromp_spanner_handover
    plt.figure(60)
    for i in range(4):
        plt.subplot(411+i)
        ipromp_spanner_handover.promps[i].plot(color='b', legend='spanner handover model, imu'); plt.axis('off')
    plt.figure(62)
    for i in range(7):
        plt.subplot(711+i)
        ipromp_spanner_handover.promps[4+i].plot(color='b', legend='spanner handover model, pose'); plt.axis('off')
    # plot ipromp_tape_hold
    plt.figure(70)
    for i in range(4):
        plt.subplot(411+i)
        ipromp_tape_hold.promps[i].plot(color='b', legend='tape hold model, imu'); plt.axis('off')
    plt.figure(72)
    for i in range(7):
        plt.subplot(711+i)
        ipromp_tape_hold.promps[4+i].plot(color='b', legend='tape hold model, pose'); plt.axis('off')
