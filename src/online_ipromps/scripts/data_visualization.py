...
for task_idx, ipromps_idx in enumerate(datasets4train_post):
    ...
    for demo_idx in range(20):
        for joint_idx in range(8):
            ax = fig.add_subplot(8, 1, 1 + joint_idx)
            plt.plot(np.linspace(0, 1.0, 101), datasets4train_post[task_idx][demo_idx]['emg'][:, joint_idx])
# plot the prior
for task_idx, ipromps_idx in enumerate(ipromps_set_post):
    fig = plt.figure(task_idx)
    fig.suptitle('emg')
    for joint_idx in range(8):
        ax = fig.add_subplot(8, 1, 1+joint_idx)
        ipromps_idx.promps[joint_idx].plot_prior()
# plot post distribution
for task_idx, ipromps_idx in enumerate(ipromps_set_post):
    print(ipromps_idx.promps[0].alpha_fit)
    fig = plt.figure(task_idx)
    for joint_idx in range(8):
        ax = fig.add_subplot(8, 1, 1 + joint_idx)
        ipromps_idx.promps[joint_idx].plot_nUpdated(color='r', via_show=True)
