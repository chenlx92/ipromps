...
ipromp_aluminum_hold = iprompslib_imu_pose.IProMP(num_joints=11, nrBasis=11, sigma=0.05, num_samples=101)
ipromp_spanner_handover = iprompslib_imu_pose.IProMP(num_joints=11, nrBasis=11, sigma=0.05, num_samples=101)
ipromp_tape_hold = iprompslib_imu_pose.IProMP(num_joints=11, nrBasis=11, sigma=0.05, num_samples=101)

# add demostration
for idx in range(0, nrDemo):
    # add demonstration of aluminum_hold
    demo_temp = np.hstack([train_set_aluminum_hold_imu_norm_full[idx], train_set_aluminum_hold_pose_norm_full[idx]])
    ipromp_aluminum_hold.add_demonstration(demo_temp)
    # add demonstration of spanner_handover
    demo_temp = np.hstack([train_set_spanner_handover_imu_norm_full[idx], train_set_spanner_handover_pose_norm_full[idx]])
    ipromp_spanner_handover.add_demonstration(demo_temp)
    # add demonstration of tape_hold
    demo_temp = np.hstack([train_set_tape_hold_imu_norm_full[idx], train_set_tape_hold_pose_norm_full[idx]])
    ipromp_tape_hold.add_demonstration(demo_temp)


# aluminum hold
test_set = np.hstack((train_set_aluminum_hold_imu_norm20, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm20;
test_set = np.hstack((train_set_aluminum_hold_imu_norm21, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm21;
test_set = np.hstack((train_set_aluminum_hold_imu_norm22, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm22;
test_set = np.hstack((train_set_aluminum_hold_imu_norm23, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm23;
test_set = np.hstack((train_set_aluminum_hold_imu_norm24, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm24;
test_set = np.hstack((train_set_aluminum_hold_imu_norm25, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm25;
test_set = np.hstack((train_set_aluminum_hold_imu_norm26, np.zeros([len_normal, 7]))); robot_response = train_set_aluminum_hold_pose_norm26;
